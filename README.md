...
- `nix run .#spawn_consumer_repo -- <name>` — open the functional-command template in `$VISUAL`/`$EDITOR`; on close, scaffold `./<name>` as a keyfarm consumer repo (git-initialized). Override the keyfarm ref with `KEYFARM_REF=`.
